...
function render() {
  ...
  $('entities').innerHTML = view.entities.map(entity => `<div class="entity"><strong>${entity.id}</strong> <span class="small">${entity.type}</span><br><span class="small">zone=${entity.zoneId ?? '-'} · hp=${entity.hp ?? '-'} · goal=${entity.goal}</span></div>`).join('');
  $('events').innerHTML = view.recentEvents.map(event => `<div class="event"><time>@${event.worldTime}</time>${event.type}</div>`).join('') || '<span class="muted">尚無事件</span>';
