...
$('activities').addEventListener('click', event => { const activityId = event.target.dataset.activity; if (!activityId) return; const result = world.startActivity(activityId); setInteractionBox(result.ok ? `已開始：${activityId}` : `無法開始：${result.reason}`); render(); });
