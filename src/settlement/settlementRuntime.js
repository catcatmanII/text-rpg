export class SettlementRuntime {
  ...
  constructor({ emit, levels = [], rationInterval = 60 } = {}) { this.emit = emit; this.prosperity = 0; this.safety = 50; this.food = 0; this.level = 1; this.levels = levels; this.shortageMinutes = 0; this.rationInterval = rationInterval; this.minutesSinceRation = 0; }
  ...
  snapshot() { return { prosperity: this.prosperity, safety: this.safety, food: this.food, level: this.level, shortageMinutes: this.shortageMinutes, minutesSinceRation: this.minutesSinceRation }; }
  restore(snapshot = {}) { Object.assign(this, { prosperity: snapshot.prosperity ?? this.prosperity, safety: snapshot.safety ?? this.safety, food: snapshot.food ?? this.food, level: snapshot.level ?? this.level, shortageMinutes: snapshot.shortageMinutes ?? 0, minutesSinceRation: snapshot.minutesSinceRation ?? 0 }); }
}
