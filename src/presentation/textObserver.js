...
export class TextObserver {
  describe(world) {
    ...
    return { worldId: world.worldId, mode: world.mode, worldMinutes: world.clock.minutes, version: world.state.version, entities, population: world.population?.stats(), statistics: world.stats?.snapshot(), economy: world.economy?.snapshot(), recentEvents: world.eventLog.events.slice(-10) };
  }
  render(world) {
