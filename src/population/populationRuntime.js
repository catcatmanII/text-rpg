export class PopulationRuntime {
  ...
  #tryBirth(worldTime) {
    ...
    this.registry.add(child); this.onAdded?.(child); this.emit?.({ type: 'NPC_BORN', entityId: child.id, parentIds: child.parentIds });
  }
  stats() {
  ...
  snapshot() { return { nextBirthAt: this.nextBirthAt, nextId: this.nextId }; }
  restore(snapshot = {}) { this.nextBirthAt = snapshot.nextBirthAt ?? 1440; this.nextId = snapshot.nextId ?? 1; }
}
